...
def generate_labels(frame, sequence_file, cnt):
    obj_name, difficulty, dimensions, locations, heading_angles = [], [], [], [], []
    tracking_difficulty, speeds, accelerations, obj_ids = [], [], [], []
    num_points_in_gt = []
    augpath = Path('/mnt/8tssd/WaymoWeatherLabels/Rain_Simulation_Data/npy')
    sequence_name = os.path.basename(str(sequence_file)).split('.')[0]
    ...
    for i in range(len(laser_labels)):
        ...
        simed_file = augpath / sequence_name / ('%03d.npy' % cnt)

        if simed_file.exists():
            x = box.center_x
            y = box.center_y
            z = box.center_z
            l = box.length
            ...
            h = box.height
            r = box.heading  
            boxes = np.array([[x, y, z, l, w, h, r]])
            points = np.load(simed_file)[:,:3]
            box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(
                    torch.from_numpy(points).unsqueeze(dim=0).float().cuda(),
                    torch.from_numpy(boxes).unsqueeze(dim=0).float().cuda()
            ).long().squeeze(dim=0).cpu().numpy()
            num_lidar_points_in_box = (box_idxs_of_pts == 0).sum()
            num_points_in_gt.append(num_lidar_points_in_box)
        else:
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, use_two_returns=True):
    ...
    for cnt, data in enumerate(dataset):
        ...
        if has_label:
            annotations = generate_labels(frame, sequence_file, cnt)
