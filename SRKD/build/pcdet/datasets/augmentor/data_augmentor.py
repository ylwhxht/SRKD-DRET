...
class DataAugmentor(object):
    ...
    def random_world_flip(self, data_dict=None, config=None):
        ...
        pair_points = np.array([[0,0,0]],dtype=np.float64)
        if 'input_dict_rain' in data_dict.keys() and data_dict['input_dict_rain'] is not None:
            input_dict_rain = data_dict['input_dict_rain']
            pair_points = input_dict_rain['points']
        
        for cur_axis in config['ALONG_AXIS_LIST']:
            assert cur_axis in ['x', 'y']
            gt_boxes, points, pair_points = getattr(augmentor_utils, 'random_flip_along_%s' % cur_axis)(
                gt_boxes, points, pair_points
            )
        if 'input_dict_rain' in data_dict.keys() and data_dict['input_dict_rain'] is not None:
            input_dict_rain['points'] = pair_points
            input_dict_rain['gt_boxes'] = gt_boxes
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        gt_boxes, points, pair_points = augmentor_utils.global_rotation(
            data_dict['gt_boxes'], data_dict['points'], rot_range=rot_range, pair_points= pair_points
        )
    ...
    def random_world_scaling(self, data_dict=None, config=None):
        ...
        gt_boxes, points, pair_points = augmentor_utils.global_scaling(
            data_dict['gt_boxes'], data_dict['points'], config['WORLD_SCALE_RANGE'], pair_points
        )
        if 'input_dict_rain' in data_dict.keys() and data_dict['input_dict_rain'] is not None:
            input_dict_rain['points'] = pair_points
            input_dict_rain['gt_boxes'] = gt_boxes
    ...
    def forward(self, data_dict):
        ...
        """
        if 'input_dict_rain' in data_dict.keys() and data_dict['input_dict_rain'] is not None:
            data_dict['input_dict_rain']['gt_boxes_mask'] = data_dict['gt_boxes_mask']
        ...
        if 'input_dict_rain' in data_dict.keys() and data_dict['input_dict_rain'] is not None:
            input_dict_rain = data_dict['input_dict_rain']
            input_dict_rain['gt_boxes'][:, 6] = common_utils.limit_period(
                input_dict_rain['gt_boxes'][:, 6], offset=0.5, period=2 * np.pi
            )

            if 'calib' in input_dict_rain:
                input_dict_rain.pop('calib')
            if 'road_plane' in input_dict_rain:
                input_dict_rain.pop('road_plane')
            if 'gt_boxes_mask' in data_dict:
                gt_boxes_mask = data_dict['gt_boxes_mask']
                input_dict_rain['gt_boxes'] = input_dict_rain['gt_boxes'][gt_boxes_mask]
                input_dict_rain['gt_names'] = input_dict_rain['gt_names'][gt_boxes_mask]
                if 'gt_boxes2d' in input_dict_rain:
                    input_dict_rain['gt_boxes2d'] = input_dict_rain['gt_boxes2d'][gt_boxes_mask]
