...
def random_flip_along_x(gt_boxes, points, pair_points = np.array([[0,0,0]])):
    """
    Args:
    ...
    if enable:
        ...
        points[:, 1] = -points[:, 1]
        pair_points[:, 1] = -pair_points[:, 1]
        if gt_boxes.shape[1] > 7:
            gt_boxes[:, 8] = -gt_boxes[:, 8]
    
    return gt_boxes, points, pair_points


def random_flip_along_y(gt_boxes, points, pair_points = np.array([[0,0,0]])):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
...
def global_rotation(gt_boxes, points, rot_range, pair_points = np.array([[0,0,0]])):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    pair_points = common_utils.rotate_points_along_z(pair_points[np.newaxis, :, :], np.array([noise_rotation]))[0]
...
def global_scaling(gt_boxes, points, scale_range, pair_points = np.array([[0,0,0]])):
    """
    Args:
