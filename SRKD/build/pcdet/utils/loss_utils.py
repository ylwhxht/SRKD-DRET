...
def get_InsRoIFeature_KD(batch_dict: dict, batch_dict_rain: dict, pairloss):
    ...
    shape = batch_dict_rain['gt_roifeatures'].shape
    feature_rain = feature_rain.reshape(batch_dict_rain['batch_size'], -1, shape[1], shape[2], shape[3], shape[4])
    feature_rain = feature_rain[rain_idx,...]
    feature_rain = feature_rain.reshape(-1, shape[1], shape[2], shape[3], shape[4])
    ...
    kd_feature_loss = (torch.mean(distance, dim = [1,2,3,4]) * weight).mean() * pairloss
...
def getKDloss(ret_dict, ret_dict_rain):
    InsRoIFeatureKD_loss = get_InsRoIFeature_KD(ret_dict, ret_dict_rain, 2)
    #print("InsRoIFeatureKD_loss", InsRoIFeatureKD_loss)
    LogitKD_loss = get_Logit_KD(ret_dict, ret_dict_rain, 300, 0.2)
    # print("InsRoIFeatureKD_loss : ",InsRoIFeatureKD_loss)
