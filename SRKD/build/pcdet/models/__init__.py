        ...
        if isinstance(val, dict):
            for k, v in val.items():
                if not isinstance(v, np.ndarray):
                    continue
                elif k in ['frame_id', 'metadata', 'calib']:
                    continue
                elif k in ['images']:
                    val[k] = kornia.image_to_tensor(v).float().cuda().contiguous()
                elif k in ['image_shape']:
                    val[k] = torch.from_numpy(v).int().cuda()
                else:
                    val[k] = torch.from_numpy(v).float().cuda()
            batch_dict[key] = val
