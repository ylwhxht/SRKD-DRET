...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.use_weatherlabels = False
        self.mask_noise = False
        self.use_PSPSim = True
        #self.augpath = Path('111')
        #self.raininfo_path = Path('111')
        if self.use_PSPSim:
            self.raininfo_path = Path('/mnt/8tssd/AdverseWeather/waymo_simed_infos/waymo/waymo_processed_data_v0_5_0')
            #self.raininfo_path = Path('/mnt/8tssd/AdverseWeather/waymo_simed_infos/waymo_LISA/waymo_processed_data_v0_5_0')
            #self.raininfo_path = Path("/mnt/8tssd/AdverseWeather/waymo_simed_infos/waymo_SPRAY/waymo_processed_data_v0_5_0/")
            self.augpath = Path('/mnt/8tssd/AdverseWeather/Rain_Simulation_Data_Del_in_Car/npy')
            #self.augpath = Path('/mnt/8tssd/AdverseWeather/LISA_Simulation')
            #self.augpath = Path("/mnt/8tssd/SPRAY/npy/")
        #self.augpath = Path('/mnt/8tssd/Domain_Adaptation/waymo_processed_data_v0_5_0/')
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            if self.raininfo_path.exists():
                info_path = self.raininfo_path / sequence_name / ('%s.pkl' % sequence_name)
    ...
    def get_lidar(self, sequence_name, sample_idx):
        ...
        withweather = Path('/mnt/8tssd/AdverseWeather/WaymoDe-noise_more info+NLZ_npy/')
        show = False
        if lidar_file.exists() and not self.val:
            #Aug  Data Path
            point_features = np.load(lidar_file)
        else :
            #weather label
            lidar_file = withweather / sequence_name / ('%03d.npy' % sample_idx)
            if   not lidar_file.exists():
    ...
    def prosess_lidar_feature(self, point_features):
        if self.use_weatherlabels:
            ...
            if self.mask_noise:
                points_all, NLZ_flag = point_features[:, [0, 1, 2, 3, 4, 6]], point_features[:, 5]
                noise_mask = points_all[:,-1] > 0
                #s = int(0.1 * len(noise_mask))
                #noise_mask[s:] = True
                points_all = points_all[noise_mask][:, :5]
                NLZ_flag = NLZ_flag[noise_mask]
            else:
...
if __name__ == '__main__':
    ...
    if args.func == 'create_waymo_infos':
        ...
        ROOT_DIR = Path("/mnt/8tssd/waymo/perception/tfrecord/")
        dataset_cfg.PROCESSED_DATA_TAG = args.processed_data_tag
        create_waymo_infos(
             dataset_cfg=dataset_cfg,
             class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
             data_path= ROOT_DIR,
             save_path= Path("/mnt/8tssd/AdverseWeather/waymo_simed_infos/waymo_SPRAY"),
