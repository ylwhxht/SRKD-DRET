...
class Waymo_WeatherDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        split_dir = self.data_path / 'ImageSets' / (self.split + '.txt')
        self.data_path = self.root_path / self.dataset_cfg.PROCESSED_DATA_TAG
        split_dir = Path('/home/hx/OpenPCDet/data/Waymo_WA') / 'ImageSets' / (self.split + '.txt')
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            info_path = Path('/home/hx/OpenPCDet/data/Waymo_WA/waymo_processed_data_train_val_test') / sequence_name / ('%s.pkl' % sequence_name)
    ...
    def find(self, file):
        bake1_data_path = Path('/mnt/DataServer/wh/data/waymo/perception/tfrecord/training/')
        bake2_data_path = Path('/mnt/DataServer/wh/data/waymo/perception/tfrecord/validation/')
        if os.path.exists(bake1_data_path / file):
            return bake1_data_path
        return bake2_data_path    
    ...
    def get_infos(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(), has_label=True, sampled_interval=1):
        ...
        raw_data_path = Path('/mnt/10tdata/xzr/waymo/domain_adaptation/training')

        sample_sequence_file_list = [
            self.check_sequence_name_with_all_version((raw_data_path if str(sequence_file).find('with')==-1 else self.find(sequence_file)) / sequence_file)
    ...
    def get_lidar(self, sequence_name, sample_idx):
        ...
        point_features = np.load(lidar_file)  # (N, 7): [x, y, z, intensity, elongation, NLZ_flag, Weather_labels]

        points_all, NLZ_flag = point_features[:, [0, 1, 2, 3, 4, 6]], point_features[:, 5]
        if not self.use_weatherlabels:
            points_all = points_all[:, :5]
        #point_features[:, 6]
    ...
    def get_lidar_noelo(self, sequence_name, sample_idx):
        #data_path = Path("/home/hx/Code/data_produce/result/waymo_txt")
        lidar_file = self.data_path / sequence_name / ('%04d.npy' % sample_idx)
        point_features = np.load(lidar_file)  # (N, 7): [x, y, z, intensity, NLZ_flag, Weather_labels]
        
        if point_features.shape[1]>5:
            points_all, NLZ_flag = point_features[:, [0, 1, 2, 3, 5]], point_features[:, 4]
            if not self.use_weatherlabels:
                points_all = points_all[:, :4]
            #point_features[:, 6]
            if not self.dataset_cfg.get('DISABLE_NLZ_FLAG_ON_POINTS', False):
                points_all = points_all[NLZ_flag == -1]
        else:
            points_all = point_features
            if not self.use_weatherlabels:
                points_all = points_all[:, :4]
        points_all[:, 3] = np.tanh(points_all[:, 3])
            ...
            for i in range(num_obj):
                ...
                if (used_classes is None) or names[i] in used_classes:
                    with open(filepath, 'w') as f:
                        gt_points.tofile(f)
                    x = np.fromfile(filepath,dtype=np.float16)
...
if __name__ == '__main__':
    # /usr/bin/python3 -m pcdet.datasets.waymo.waymo_dataset_weather --func create_waymo_infos --cfg_file tools/cfgs/dataset_configs/waymo_dataset_WA_noelo.yaml
    import argparse

    parser = argparse.ArgumentParser(description='arg parser')
    parser.add_argument('--cfg_file', type=str, default=None, help='specify the config of dataset')
    parser.add_argument('--func', type=str, default='create_waymo_infos', help='')
    parser.add_argument('--processed_data_tag', type=str, default='waymo_processed_data_train_val_test', help='')
    ...
    if args.func == 'create_waymo_infos':
        ...
        ROOT_DIR = Path("/home/hx/OpenPCDet/data/Waymo_WA")
        dataset_cfg.PROCESSED_DATA_TAG = args.processed_data_tag
        create_waymo_infos(
             dataset_cfg=dataset_cfg,
             class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
             data_path= ROOT_DIR,
             save_path= Path('/home/hx/OpenPCDet/data/Waymo_WA'),
