    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                if str(file_path).find('Rain_data')!=-1:
                    obj_points = np.fromfile(str(file_path), dtype=np.float16).reshape([-1, self.sampler_cfg.NUM_POINT_FEATURES])
        ...
        if obj_points.shape[1] != points.shape[1]:
            obj_points = np.concatenate([obj_points,np.ones(obj_points.shape[0], dtype = np.float16).reshape(-1, 1)], axis = 1)
    ...
    def __call__(self, data_dict):
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            if 'input_dict_rain' in data_dict.keys() and data_dict['input_dict_rain'] is not None:
                ...
                input_dict_rain = self.add_sampled_boxes_to_scene(input_dict_rain, sampled_gt_boxes, total_valid_sampled_dict)
                data_dict['input_dict_rain'] = input_dict_rain
            
        if 'input_dict_rain' in data_dict.keys() and data_dict['input_dict_rain'] is not None:
            data_dict['input_dict_rain'].pop('gt_boxes_mask')
