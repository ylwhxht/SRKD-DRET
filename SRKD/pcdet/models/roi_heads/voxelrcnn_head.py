...
class VoxelRCNNHead(RoIHeadTemplate):
    ...
    def gt_roi_grid_pool(self, batch_dict):
        """
        Args:
            batch_dict:
                batch_size:
    ...
    def forward(self, batch_dict):
        ...
        if self.training and ('teacher' in batch_dict.keys() or ('rain_idx' in batch_dict.keys() and len(batch_dict['rain_idx']) > 0)):
            gt_pooled_features = self.gt_roi_grid_pool(batch_dict)  # (BxN, 6x6x6, C)

            batch_dict['gt_roifeatures'] = gt_pooled_features.view(gt_pooled_features.size(0), -1)
