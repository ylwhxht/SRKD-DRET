...
from pcdet.utils import kd_utils 
...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    if rank == 0:
        ...
        de_loss_disp = common_utils.AverageMeter()
        Ins_loss_disp = common_utils.AverageMeter()
        Rsp_loss_disp = common_utils.AverageMeter()
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        if rank == 0:
            ...
            loss_disp.update(tb_dict['lossval'])
            if tb_dict['haspair']:
                if 'Ins' not in tb_dict.keys():
                    tb_dict['Ins'] = -1
                if 'Rsp' not in tb_dict.keys():
                    tb_dict['Rsp'] = -1
                if 'denoise_loss' not in tb_dict.keys():
                    tb_dict['denoise_loss'] = -1
                Ins_loss_disp.update(tb_dict['Ins'])
                Rsp_loss_disp.update(tb_dict['Rsp'])
                de_loss_disp.update(tb_dict['denoise_loss'])
            
            disp_dict.update({
                'loss_avg': f'{loss_disp.val:.2f}({loss_disp.avg:.2f})',
                'SRKD_loss': f'{de_loss_disp.val:.2f}+{Ins_loss_disp.val:.2f}+{Rsp_loss_disp.val:.2f}({de_loss_disp.avg:.2f}+{Ins_loss_disp.avg:.2f}+{Rsp_loss_disp.avg:.2f})', 
                'lr': cur_lr, 
                'd_t': f'({data_time.avg:.1f})',
...
def train_model_kd(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        ckpt_name = ckpt_save_dir / ('checkpoint_epoch_%d.pth' % start_epoch)
        if self_KD:
            print('update teacher from ' + str(ckpt_name))
            loc_type = torch.device('cpu')
            checkpoint = torch.load(ckpt_name, map_location=loc_type)
            teacher_model._load_state_dict(checkpoint['model_state'], strict=True)
        ...
        for cur_epoch in tbar:
            ...
            if self_KD:
                print('update teacher from ' + str(ckpt_name))
                loc_type = torch.device('cpu')
                checkpoint = torch.load(ckpt_save_dir / ('checkpoint_epoch_%d.pth' % trained_epoch), map_location=loc_type)
                teacher_model._load_state_dict(checkpoint['model_state'], strict=True)