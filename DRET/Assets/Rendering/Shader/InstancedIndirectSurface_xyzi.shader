Shader "Instanced/InstancedShader_xyzi"
{
	Properties
	{
	...
	SubShader
	{
		Pass
		{
		...
		#if SHADER_TARGET >= 45
			StructuredBuffer<float4> positionInfoBuffer;
		#endif
			...
			{
			#if SHADER_TARGET >= 45
				float4 data = positionInfoBuffer[instanceID];
			#else
				float4 data = 0;
			#endif
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float3 rgb = float3(1.0f, 1.0f - i.intensity, 1.0f - i.intensity);
