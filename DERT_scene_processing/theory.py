...
def P_R_rain_hard(p: ParameterSet, R: float) -> float:

    return np.exp(-2 * p.alpha * p.r_0) * P_R_clear_hard(p, R)


def P_R_rain_soft(p, R: float, n: int = None) -> float:
...
def P_R_rain(p: ParameterSet, R: float, n: int = None) -> float:

    if n is None:
        n = p.n

    return P_R_rain_soft(p, R, n) + P_R_rain_hard(p, R)
