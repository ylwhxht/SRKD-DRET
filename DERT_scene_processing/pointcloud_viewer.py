...
parser.add_argument('-d', '--datasets', type=str, help='path to where you store your datasets',
                    default=str('/mnt/8tssd/kitti/detection/'))
...
KITTI = DATASETS_ROOT / 'training/velodyne'
...
class MyWindow(QMainWindow):
    ...
    def show_pointcloud(self, filename: str) -> None:
        ...
        if self.simulated_fog and self.success and 'extraction' not in filename:
            ...
            pc = self.load_pointcloud('/mnt/8tssd/kitti/detection/training/velodyne/000000.bin')

            pc, simulated_fog_pc, info_dict = simulate_fog(self.p, pc, self.noise, self.gain,self.noise_variant)
